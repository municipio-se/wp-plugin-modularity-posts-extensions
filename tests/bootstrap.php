<?php

declare(strict_types=1);

define('ABSPATH', __DIR__);
define('MODULARITY_PATH', '/theme/Modularity/');
define('MODULARITY_POSTS_EXTENSIONS_PATH', dirname(__DIR__) . '/');
define('MODULARITY_POSTS_EXTENSIONS_URL', 'https://example.test/wp-content/plugins/modularity-posts-extensions/');
define('MODULARITY_POSTS_EXTENSIONS_VERSION', '0.1.0');

$GLOBALS['modularity_posts_extensions_test_actions'] = [];
$GLOBALS['modularity_posts_extensions_test_filters'] = [];
$GLOBALS['modularity_posts_extensions_test_styles'] = [];
$GLOBALS['modularity_posts_extensions_test_textdomains'] = [];
$GLOBALS['modularity_posts_extensions_test_list_template_modules'] = [];

function add_action(string $hook, callable $callback, int $priority = 10, int $acceptedArgs = 1): void
{
    $GLOBALS['modularity_posts_extensions_test_actions'][] = [$hook, $callback, $priority, $acceptedArgs];
}

function add_filter(string $hook, callable $callback, int $priority = 10, int $acceptedArgs = 1): void
{
    $GLOBALS['modularity_posts_extensions_test_filters'][] = [$hook, $callback, $priority, $acceptedArgs];
}

function load_plugin_textdomain(string $domain, bool $deprecated, string $path): void
{
    $GLOBALS['modularity_posts_extensions_test_textdomains'][] = [$domain, $deprecated, $path];
}

function wp_enqueue_style(...$args): void
{
    $GLOBALS['modularity_posts_extensions_test_styles'][] = $args;
}

function _x(string $text): string
{
    return $text;
}

function esc_attr(string $text): string
{
    return htmlspecialchars($text, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
}

function esc_html(string $text): string
{
    return htmlspecialchars($text, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
}

require __DIR__ . '/support/modularity-stubs.php';
require dirname(__DIR__) . '/vendor/autoload.php';
