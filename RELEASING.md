...
Version
[1.0.0](https://github.com/municipio-se/wp-plugin-modularity-posts-extensions/releases/tag/1.0.0)
...
Verify zero, one, two and multiple posts, ordering, archive links, card fields
and accessible list links. Other Posts formats must remain unchanged.

The pre-release suite passed on PHP 8.3.33 (19 tests / 41 assertions) on
...
5. In each approved consumer, require
   municipio/wp-plugin-modularity-posts-extensions:^1.0, review the dependency
   diff, commit the lockfile, deploy after approval and repeat the relevant
   reference checks. Retain the previous lockfile and deployment reference for
   rollback.

## Rollback

No activation or write migration runs. Roll back the package and lockfile; the
saved mixed value and field metadata remain available.
